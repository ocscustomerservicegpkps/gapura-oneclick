-- Fixes "column reference \"report_type\" is ambiguous" thrown by
-- upsert_report_document_bundle on every finalize call.
--
-- RETURNS TABLE (report_type text, report_id text, ...) implicitly declares
-- report_type/report_id as PL/pgSQL variables in the function body's scope
-- (Postgres OUT-parameter behavior). Those names collide with the identical
-- column names on public.report_documents, and PL/pgSQL's default
-- variable-resolution rule ("use_variable") makes any unqualified reference
-- to report_type/report_id inside embedded SQL ambiguous — most visibly in
-- ON CONFLICT (report_type, report_id), whose conflict-target column list
-- can't be alias-qualified to disambiguate it the way rd.report_type can be.
--
-- #variable_conflict use_column tells PL/pgSQL to prefer the table column
-- over the same-named OUT variable whenever a plain identifier is ambiguous,
-- which is exactly the collision here — no need to rename the RETURNS TABLE
-- columns the TypeScript caller (lib/report-documents-server.ts) depends on.
