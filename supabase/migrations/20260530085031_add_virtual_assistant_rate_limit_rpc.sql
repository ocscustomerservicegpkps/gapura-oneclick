-- Atomic persistent rate-limit consume helper for server-side API callers.
-- Uses existing public.rate_limits table and keeps execution restricted to service role.
...
BEGIN
    ...
    IF p_reset_at IS NULL THEN
        RAISE EXCEPTION 'p_reset_at is required';
    END IF;

    DELETE FROM public.rate_limits AS rl WHERE rl.reset_at < now();
